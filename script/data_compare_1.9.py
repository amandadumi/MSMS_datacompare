    ...
    for m in range(len(a)-3):
        ...
        for i in range(len(df['collected'])):
            ...
            for j in range(stuff):
                if unittype == 'Da':
                    error=np.abs((df.iloc[i,0])-df.iloc[j,4])
                elif unittype == 'ppm':
...
def check_unittype(unittype):
    correct_units = False
    while correct_units == False:
        if unittype == 'Da' or unittype == 'ppm':
            correct_units = True
        else:
            unittype = str(input('Units(ppm/Da): '))
    return unittype
